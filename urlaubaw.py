...
st.title("Urlaubs-Meldungen Monatsübersicht (ab 2025)")
    ...
    if eintraege:
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            for monat_key in sorted(df_gesamt["Monat"].unique()):
                ...
                # Anzahl Urlaubstage pro Name in diesem Monat berechnen
                urlaubstage_pro_name = (
                    df_monat.groupby(["Nachname", "Vorname"]).size().to_dict()
                )
                zeilen = []
                for (nach, vor), gruppe in df_monat.groupby(["Nachname", "Vorname"]):
                    urlaubstage = urlaubstage_pro_name.get((nach, vor), 0)
                    zeilen.append([f"{vor} {nach} ({urlaubstage} Tage)", ""])
            ...
            df_uebersicht = (
                ...
                .size()
                .reset_index(name="Anzahl Urlaubs-Meldungen")
                .sort_values(by="Anzahl Urlaubs-Meldungen", ascending=False)
            )

            # Zeile für die Überschrift, dann Leerzeile, dann Spaltenüberschriften und Tabelle
            ueberschrift = [["Urlaubs-Meldungen Übersicht ab 2025"], [""], ["Nachname", "Vorname", "Anzahl Urlaubs-Meldungen"]]
        ...
        st.download_button("Excel-Datei herunterladen", output.getvalue(), file_name="Urlaub_Monatsauswertung.xlsx")

    else:
        st.warning("Keine gültigen Urlaubs-Meldungen ab 2025 gefunden.")
